...
Herald 具有远程终端控制能力，安全必须作为一等公民对待。
...
```
手机 ──WSS (TLS 1.3)──► 服务器 ──WSS (TLS 1.3)──► Mac
```

- 服务器使用 Let's Encrypt 自动续签证书
- Mac 客户端验证服务器证书，防止 MITM
- 禁用 TLS 1.2 及以下版本
...
│                   安全分层                        │
│                                                 │
│  L5  会话安全    自动锁定 / 操作日志 / 密钥轮换     │
│  L4  权限控制    危险命令拦截 + Mac 弹窗确认        │
│  L3  设备认证    X25519 密钥交换 + 一次性配对码     │
│  L2  端到端加密  ChaCha20-Poly1305 (服务器不可读)  │
│  L1  传输加密    全链路 WSS (TLS 1.3)             │
└─────────────────────────────────────────────────┘
```
