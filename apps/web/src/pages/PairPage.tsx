...
export function PairPage() {
  ...
  const doPair = useCallback(
    async (code: string) => {
      ...
      const deviceName = /iPhone/i.test(navigator.userAgent)
        ? 'iPhone'
    ...
    <div className="flex flex-col items-center justify-center min-h-screen px-6 bg-bg-primary">
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-2xl font-bold text-text-primary">AirClaude</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            ...
            maxLength={6}
            autoFocus
            className="w-full px-4 py-4 bg-bg-secondary border border-border rounded-[var(--radius-card)] text-center text-2xl font-[family-name:var(--font-mono)] tracking-[0.3em] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent-blue transition-colors"
          />
          ...
          <button
            ...
            className="w-full py-3.5 bg-accent-blue text-white rounded-[var(--radius-card)] font-semibold text-sm disabled:opacity-40 active:scale-[0.98] transition-all"
          >
            {loading ? '配对中...' : '连接'}
          </button>
        </form>
